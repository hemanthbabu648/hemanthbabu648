...
export default function ContactCodeBlock({
  ...
  return (
    <div className="bg-[var(--bg-secondary)]/60 backdrop-blur-sm rounded-lg p-4 border border-[var(--border-muted)] font-mono text-sm">
      <div className="text-[var(--text-muted)]">
        <span className="text-[var(--accent-purple)]">const</span>{' '}
        <span className="text-[var(--accent-cyan)]">{variableName}</span> = {'{'}
      </div>
      {properties.map((prop, index) => (
        <div key={prop.key} className="text-[var(--text-muted)] ml-4">
